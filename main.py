    ...
    try:
        ...
        if has_alpha:
            ...
            upscaled_alpha = cv2.resize(
                alpha, (new_width, new_height), interpolation=cv2.INTER_LANCZOS4
            )
...
def remove_background_ai(image_array):
    ...
    try:
        ...
        print(
            f"  Warning: AI removal failed ({str(e)}), falling back to color-based method"
        )
        return None, False
...
def get_object_bounds(image_array):
    """
...
def finalize_portrait(image_array, target_height=1500, aspect_ratio=(3, 5)):
    """
    Finalize portrait image:
    - Scale object to target height
    - Create 3:5 aspect ratio canvas
    - Position at bottom with margins (10px top, 10px min sides)
    - Center horizontally

    Args:
        image_array: RGBA image array
        target_height: Target height for the object in pixels
        aspect_ratio: Target aspect ratio as (width, height)

    Returns:
        Finalized image array
    """
    # Get object bounds
    bounds = get_object_bounds(image_array)
    if bounds is None:
        return image_array

    x, y, obj_width, obj_height = bounds

    # Crop to object bounds
    cropped = image_array[y : y + obj_height, x : x + obj_width]

    # Calculate scale to reach target height
    scale = target_height / obj_height
    new_obj_height = target_height
    new_obj_width = int(obj_width * scale)

    # Resize object
    pil_obj = Image.fromarray(cropped)
    pil_obj_resized = pil_obj.resize(
        (new_obj_width, new_obj_height), Image.Resampling.LANCZOS
    )
    obj_resized = np.array(pil_obj_resized)

    top_margin = 10
    side_margin = 10

    min_canvas_width = new_obj_width + 2 * side_margin

    canvas_height = int(min_canvas_width * aspect_ratio[1] / aspect_ratio[0])
    canvas_width = min_canvas_width

    if canvas_height < new_obj_height + top_margin:
        canvas_height = new_obj_height + top_margin + 10
        canvas_width = int(canvas_height * aspect_ratio[0] / aspect_ratio[1])

    canvas = np.zeros((canvas_height, canvas_width, 4), dtype=np.uint8)

    x_pos = (canvas_width - new_obj_width) // 2
    y_pos = canvas_height - new_obj_height

    if y_pos < top_margin:
        additional_height = top_margin - y_pos
    ...
    canvas[y_pos : y_pos + new_obj_height, x_pos : x_pos + new_obj_width] = obj_resized
    ...
    try:
        ...
        if bg_color == "other":
            ...
            if ai_success:
                ...
                result_image = remove_background_color_based_enhanced(
                    image_rgb, bg_color
                )
                method_used = "Color-based removal"
                steps.append("✓ Color-based removal")

        # Step 4: Upscale if needed (before finalization)
        if pbar:
        ...
        if current_max < 800:
            ...
            result_image = intelligent_upscale(result_image, max_dimension=1000)
            final_height, final_width = result_image.shape[:2]
            steps.append(f"✓ Upscaled to {final_width}x{final_height}px")
        else:
            steps.append(f"✓ Size good ({width}x{height}px)")

        # Step 5: Finalize portrait (3:5 ratio, bottom alignment, margins)
        if pbar:
            pbar.set_description("🖼️  Finalizing portrait")
        final_image_array = finalize_portrait(result_image, target_height=1500)
        final_height, final_width = final_image_array.shape[:2]
        steps.append(f"✓ Finalized to {final_width}x{final_height}px (3:5 ratio)")
